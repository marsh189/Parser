...
ifAction boolCalc firstAction secondAction = do
  condition <- boolCalc
  if condition then
    firstAction
  else
...
boolCalc (Reln cOp expr1 expr2) = do
  n1 <- (intCalc expr1) 
  n2 <- (intCalc expr2)
  let func = (opLookup cOp relationalOps)
  return $ func n1 n2
boolCalc (BBin op expr1 expr2) = do
  n1 <- (boolCalc expr1)
  n2 <- (boolCalc expr2)
  let func = (opLookup op binOps)
  return $ func n1 n2
boolCalc (BUn op expr) = do
  n1 <- (boolCalc expr)
  let func = (opLookup op binNegOps)
  return $ func n1
